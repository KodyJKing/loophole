...
export default class Editor {
    ...
    drawSelectionBar() {
        ...
        for ( let i = -4; i <= 4; i++ ) {
            ...
            let k = GMath.modulus( i + j, LoopholeTiles.length )
            let dx = this.selectionSmooth - j
            let tile = LoopholeTiles[ k ]
            canvas.push().translate( ( i - dx ) * stride - Tile.width / 2, - Tile.width / 2 )
            let scale = Tile.width / ( tile.image( world, 0, 0, 0 )?.width ?? Tile.width )
    ...
    get selectedTile() {
        return LoopholeTiles[ this.selectionIndex ]
    }

    get selectionIndex() {
        return GMath.modulus( this.selection, LoopholeTiles.length )
    }
}